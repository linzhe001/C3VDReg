...
class GeometricStructureEmbedding(nn.Module):
    ...
    def get_embedding_indices(self, points):
        ...
        # Some low-overlap C3VD batches collapse to very small coarse point sets.
        # Clamp angular-neighbor lookup to the available population so full-train
        # validation does not fail when the vendor 3DMatch angle_k is too large.
        k = min(self.angle_k, max(num_point - 1, 0))
        if k == 0:
            a_indices = d_indices.new_zeros(batch_size, num_point, num_point, 1)
