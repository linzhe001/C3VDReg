...
def _draw_for_notebook(geometries):
    """Lazily import the notebook visualizer to avoid headless side effects."""
    try:
        from open3d.web_visualizer import draw
    except Exception as exc:  # pragma: no cover - optional visualization path
        raise RuntimeError("Open3D web visualizer is unavailable") from exc
    draw(geometries)
        ...
        for i, data in tqdm.tqdm(enumerate(testloader), total=len(testloader), ncols=73, leave=False):
            ...
            for j in range(start_idx, end_idx):
                ...
                if vis:
                    ...
                    if j == 0:
                        if toyexample:
                            _draw_for_notebook([pcd1, pcd2])
                        else:
                            o3d.visualization.draw_geometries([pcd1, pcd2])
                    else:
                        if toyexample:
                            _draw_for_notebook([pcd0, pcd1])
                        else:
