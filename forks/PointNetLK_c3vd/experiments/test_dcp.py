...
class RigidTransform(object):
    ...
    def __call__(self, pcloud):
        ...
        x = (
            torch.from_numpy(self.perturbations[idx, :]).contiguous().view(1, 6)
        )  # [1, 6]
        x = x.to(pcloud)
...
def options(argv=None):
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="PATH",
        help="path to trained DCP model file",
    )
    ...
    parser.add_argument(
        "--head", type=str, default="mlp", choices=["mlp", "svd"], help="head network"
    )
    ...
    parser.add_argument(
        "--ff-dims", type=int, default=1024, help="transformer feed-forward dims"
    )
    parser.add_argument(
        "--dropout", type=float, default=0.0, help="transformer dropout"
    )

    # 1. 添加 'cycle' 参数以修复 AttributeError
    # 1. Add 'cycle' parameter to fix AttributeError
    parser.add_argument(
        "--cycle", action="store_true", help="use cycle consistency loss (for DCP)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="Maximum number of test samples (0 for no limit)",
    )
...
def main(args):
    ...
    if args.perturbation_file and os.path.exists(args.perturbation_file):
        if args.perturbation_file not in perturbation_files:
            ...
            print(
                f"Added single perturbation file: {os.path.basename(args.perturbation_file)}"
            )

    if not perturbation_files:
        print(
            "Error: No perturbation files found. Please use --perturbation-dir or --perturbation-file."
        )
        return
...
class Action:
    ...
    def eval_1__write(self, fout, ig_gt, g_hat, sample_info=None):
        ...
        for i in range(g_hat.size(0)):
            sample_id = (
                sample_info.get("identifier", f"sample_{i}")
                if sample_info
                else f"sample_{i}"
            )
            scene_name = (
                sample_info.get("scene", "unknown") if sample_info else "unknown"
            )
            sequence = (
                sample_info.get("sequence", f"{i:04d}") if sample_info else f"{i:04d}"
            )
    ...
    def eval_1(self, model, testloader, device):
        ...
        with open(self.filename, "w") as fout:
            ...
            print(
                f"# Perturbation file: {os.path.basename(self.args.perturbations)}",
                file=fout,
            )
            ...
            with torch.no_grad():
                for i, data in enumerate(testloader):
                    ...
                    try:
                        ...
                        sample_info = {}
                        if self.args.dataset_type == "c3vd" and hasattr(
                            testloader.dataset, "get_cloud_info"
                        ):
                            sample_info = testloader.dataset.get_cloud_info(i)
                        else:
                            sample_info = {
                                "identifier": f"modelnet_sample_{i}",
                                "scene": "modelnet",
                                "sequence": f"{i:04d}",
                            }
...
def get_datasets(args):
    ...
    if args.dataset_type == "modelnet":
        if not fmt_trans:
            print(
                "INFO: Twist vector format ('wv') detected. Converting to Euler angles and translation for ModelNet loader."
            )
            ...
            print(
                "INFO: Converted perturbations to Euler angles (degrees) and translation."
            )

        # Directly use the modified ModelNet40 loader
        testset = ModelNet40(
            num_points=args.num_points, partition="test", perturbations=perturbations
        )
        ...
        transform = torchvision.transforms.Compose(
            []
        )  # No specific transform needed here
    ...
    if args.max_samples and args.max_samples > 0:
        if len(testset) > args.max_samples:
            print(
                f"Limiting test set from {len(testset)} to {args.max_samples} samples."
            )
            # 使用随机索引而不是顺序索引
