...
def options(argv=None):
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="PATH",
        help="path to the perturbations",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="max-iter on ICP. (default: 20)",
    )
...
def main(args):
    ...
    if not perturbation_files:
        print(
            "错误: 没有找到扰动文件。请使用 --perturbation-dir 指定扰动文件夹或使用 --perturbations 指定扰动文件。"
        )
        return
    ...
    for i, pert_file in enumerate(perturbation_files):
        ...
        if angle_str:
            ...
            current_outfile = os.path.join(
                output_dir, f"icp_results_{base_filename}.log"
            )
...
def run(args, testset, action):
    ...
    class DatasetWrapper(torch.utils.data.Dataset):
        ...
        def test_all_indices(self):
            ...
            print(
                f"数据集有效性检查完成: {len(self.valid_indices)}/{len(self.dataset)} 有效"
            )
        ...
        def __getitem__(self, idx):
            try:
                ...
                # 如果没有有效样本，则创建一个空点云
                    ...
                    random_cloud = torch.from_numpy(
                        np.random.rand(self.num_points, 3).astype(np.float32)
                    )
...
class Action:
    ...
    def eval_1(self, testloader):
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(testloader):
                    ...
                    try:
                        ...
                        dg = torch.matmul(
                            g_hat, ig_gt
                        )  # if correct, dg == identity matrix.
                        dx = ptlk.se3.log(
                            dg
                        )  # --> [1, 6] (if corerct, dx == zero vector)
        ...
        print(f"\n====== 测试完成 ======")
        print(
            f"总耗时: {total_time:.2f}秒 (平均每样本 {total_time / total_count:.2f}秒)"
        )
        print(
            f"成功处理样本: {success_count}/{total_count} ({success_count / total_count * 100:.1f}%)"
        )
        ...
        if error_count > 0:
            print(
                f"配准失败样本: {error_count}/{success_count} ({error_count / success_count * 100:.1f}%)"
            )
    ...
    if args.dataset_type == "modelnet":
            ...
            if os.path.exists(source_root):
                ...
                if scenes:
                    for scene in scenes[
                        ...
                        print(
                            f"场景 {scene} 中的文件: {files[:5]}{'...' if len(files) > 5 else ''}"
                        )
        ...
        class FixedTransformSE3(ptlk.data.transforms.RandomTransformSE3):
            def __init__(self, perturbations, fmt_trans=False):
                super().__init__(
                    0.8, False
                )  # 随机参数在这里不重要，我们会覆盖__call__ # Random parameters don't matter here, we will override __call__
                self.perturbations = perturbations
                self.fmt_trans = fmt_trans
                self.igt = None  # 初始化igt属性 # Initialize igt attribute
                self.idx = (
                    0  # 当前使用的扰动索引 # Current perturbation index being used
                )
            ...
            def __call__(self, points):
                ...
                twist = (
                    torch.from_numpy(self.perturbations[self.idx])
                    .contiguous()
                    .view(1, 6)
                )
