...
from ptlk.data.datasets import (
    ModelNet,
    ShapeNet2,
    C3VDDataset,
    C3VDset4tracking,
    C3VDset4tracking_test,
    VoxelizationConfig,
)

# from ptlk.data.datasets import SinglePairDataset, SinglePairTrackingDataset  # 新增导入
from ptlk.data.datasets import (
    C3VDset4tracking_test_random_sample,
    CADset4tracking_fixed_perturbation_random_sample,
)
...
def options(argv=None):
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="PATH",
        help="path to the perturbation file",
    )  # see. generate_perturbations.py

    # settings for input data
    parser.add_argument(
        "--dataset-type",
        default="modelnet",
        choices=["modelnet", "c3vd"],
        metavar="DATASET",
    ...
    )  # the output is always in twist format
    parser.add_argument(
        "--num-points",
        default=1024,
        type=int,
    ...
    parser.add_argument(
        "--pair-mode",
        default="one_to_one",
        choices=["one_to_one", "scene_reference"],
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="max-iter on LK. (default: 20)",
    )
    parser.add_argument(
        "--pretrained",
        default="",
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="PATH",
        help="path to classifier feature (default: null (no-use))",
    )
    parser.add_argument(
        "--dim-k",
        default=1024,
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="Number of attention blocks in attention module (default: 3)",
    )
    parser.add_argument(
        "--num-heads",
        default=8,
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="Number of Mamba blocks in Mamba3D module (default: 3)",
    )
    parser.add_argument(
        "--d-state",
        default=16,
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="Number of attention blocks in fast point attention module (default: 2)",
    )
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="Scale factor for fast point attention model (default: 1, larger values mean lighter model)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="Number of proxy points in Cformer model (default: 8)",
    )
    parser.add_argument(
        "--num-blocks",
        default=2,
        type=int,
        metavar="N",
        help="Number of blocks in Cformer model (default: 2)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="Maximum number of test samples (default: 2000)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="PATH",
        help="Perturbation directory path, will process all perturbation files in the directory",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="PATH",
        help="Single perturbation file path (e.g., gt_poses.csv)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="VALUES",
        help="单行扰动值，逗号分隔 (Single perturbation values, comma-separated). 格式: rx,ry,rz,tx,ty,tz",
    )
    parser.add_argument(
        "--enhanced-output",
        action="store_true",
...
def main(args):
    ...
    if args.single_pair_mode:
        ...
        if not args.source_cloud or not args.target_cloud:
            ...
            print(
                "Error: Single pair mode requires --source-cloud and --target-cloud parameters"
            )
            return
    ...
    if args.perturbations and os.path.exists(args.perturbations):
        if args.perturbations not in perturbation_files:
            ...
            print(
                f"Added individually specified perturbation file: {os.path.basename(args.perturbations)}"
            )
    ...
    if args.perturbation_file and os.path.exists(args.perturbation_file):
        if args.perturbation_file not in perturbation_files:
            ...
            print(
                f"Added perturbation file: {os.path.basename(args.perturbation_file)}"
            )
    ...
    if not perturbation_files:
        print(
            "Error: No perturbation files found. Please use --perturbation-dir to specify perturbation directory, --perturbations to specify perturbation file, or --perturbation-file to specify perturbation file."
        )
        return
    ...
    for i, pert_file in enumerate(perturbation_files):
        ...
        is_single_file = len(perturbation_files) == 1 and (
            args.perturbations == pert_file or args.perturbation_file == pert_file
        )
        ...
        if angle_str:
            ...
            output_dir = os.path.join(
                os.path.dirname(args.outfile), f"angle_{angle_str}"
            )
...
def run(args, testset, action):
    ...
    class DatasetWrapper(torch.utils.data.Dataset):
        ...
        def pre_check_dataset(self):
            ...
            print(
                f"Checking {check_count} samples out of {total_samples} total samples..."
            )
            ...
            for idx in range(check_count):
                try:
                    ...
                    if not isinstance(p0, torch.Tensor) or not isinstance(
                        p1, torch.Tensor
                    ):
                    ...
                    if igt is None or not isinstance(igt, torch.Tensor):
                        print(
                            f"Warning: Sample {idx} has invalid transformation matrix"
                        )
                        continue
                    ...
                    if (
                        not torch.isfinite(p0).all()
                        or not torch.isfinite(p1).all()
                        or not torch.isfinite(igt).all()
                    ):
            ...
            print(
                f"Validation completed: {valid_count}/{check_count} samples are valid"
            )
            ...
            if valid_count > 0:
                print(
                    f"Dataset appears healthy, assuming all {total_samples} samples are usable"
                )
                self.valid_indices = list(range(total_samples))
            else:
                print(
                    f"Warning: No valid samples found in initial check, but continuing with full dataset"
                )
                self.valid_indices = list(range(total_samples))
        ...
        def get_cloud_info(self, idx):
            ...
            try:
                ...
                try:
                    ...
                    elif hasattr(self.dataset, "dataset") and hasattr(
                        self.dataset.dataset, "pairs"
                    ):
                        if hasattr(self.dataset, "indices"):
                            orig_idx = self.dataset.indices[real_idx]
                            if orig_idx < len(self.dataset.dataset.pairs):
                                source_file, target_file = self.dataset.dataset.pairs[
                                    orig_idx
                                ]
                except Exception as e:
        ...
        def get_original_clouds(self, idx):
            ...
            try:
                ...
                if hasattr(self.dataset, "get_original_clouds"):
                    original_source, original_target = self.dataset.get_original_clouds(
                        real_idx
                    )
    ...
    testloader = torch.utils.data.DataLoader(
        ...
        num_workers=min(
            args.workers, 1
        ),  # 减少worker数量，降低出错率 # Reduce worker count to lower error rate
    ...
    print(
        f"Perturbation file test evaluation completed, successfully processed {success_count}/{total_count} samples"
    )
...
class Action:
    def __init__(self, args):
        ...
        # 可视化文件夹
        if (
            self.visualize_pert is not None
            and self.current_pert_file in self.visualize_pert
        ):
            vis_dir = os.path.join(os.path.dirname(self.filename), "visualize")
            os.makedirs(vis_dir, exist_ok=True)
            # 为当前扰动文件创建子文件夹
            self.vis_subdir = os.path.join(
                vis_dir, os.path.splitext(self.current_pert_file)[0]
            )
            ...
            with open(self.vis_log_file, "w") as f:
                ...
                f.write(
                    "点云对,扰动文件,预测值(w1,w2,w3,v1,v2,v3),真实扰动(w1,w2,w3,v1,v2,v3)\n"
                )
                f.write(
                    "--------------------------------------------------------------------\n"
                )
        ...
        if self.model_type == "attention":
            ...
            if self.transfer_from and os.path.isfile(self.transfer_from):
                try:
                    ...
                    if any(
                        key.startswith("features.") for key in pretrained_dict.keys()
                    ):
                        ...
                        for key, value in pretrained_dict.items():
                            if key.startswith("features."):
                                ...
                                new_key = key[
                                    9:
                                ]  # 'features.' 有9个字符 # 'features.' has 9 characters
                        ...
                        print(
                            f"成功从分类器权重中提取并加载PointNet特征权重: {self.transfer_from}"
                        )
                    else:
                        # 直接加载特征提取器权重
    ...
    def eval_1__write(self, fout, ig_gt, g_hat, sample_info=None):
        ...
        trans_norm = trans_error.norm(
            p=2, dim=1
        )  # 平移误差L2范数 # Translation error L2 norm
    ...
    def eval_1(self, model, testloader, device):
        ...
        need_visualization = (
            self.visualize_pert is not None
            and self.current_pert_file in self.visualize_pert
            and current_vis_count < self.visualize_samples
        )
        ...
        if need_visualization:
            ...
            vis_indices = random.sample(
                range(num_samples), min(num_samples, self.visualize_samples)
            )
            print(f"\n====== Visualization Settings ======")
            print(
                f"Will visualize the following sample indices for perturbation file {self.current_pert_file}: {vis_indices}"
            )
                    ...
                    try:
                        ...
                        if i > 0 and i % 500 == 0:
                            if torch.cuda.is_available():
                                ...
                                print(
                                    f"Sample {i}: GPU memory - Allocated: {memory_allocated:.2f}GB, Reserved: {memory_reserved:.2f}GB"
                                )
                            else:
                        ...
                        # 检查点云形状和有效性
                        if (
                            p0.shape[1] != self.num_points
                            or p1.shape[1] != self.num_points
                        ):
                            print(
                                f"警告: 批次 {i} 点云形状不符合预期: p0={p0.shape}, p1={p1.shape}, 期望点数={self.num_points}"
                            )
                        ...
                        if not sample_info:
                            try:
                                # 尝试从数据集中获取源文件和目标文件路径
                                if hasattr(testloader.dataset, "dataset") and hasattr(
                                    testloader.dataset.dataset, "pairs"
                                ):
                                    idx = (
                                        testloader.dataset.valid_indices[i]
                                        if hasattr(testloader.dataset, "valid_indices")
                                        else i
                                    )
                                    if idx < len(testloader.dataset.dataset.pairs):
                                        source_file_path, target_file_path = (
                                            testloader.dataset.dataset.pairs[idx]
                                        )
                                elif hasattr(testloader.dataset, "dataset") and hasattr(
                                    testloader.dataset.dataset, "dataset"
                                ):
                                    if hasattr(
                                        testloader.dataset.dataset.dataset, "pairs"
                                    ):
                                        idx = (
                                            testloader.dataset.valid_indices[i]
                                            if hasattr(
                                                testloader.dataset, "valid_indices"
                                            )
                                            else i
                                        )
                                        if idx < len(
                            ...
                            if source_file_path:
                                try:
                                    ...
                                    if (
                                        basename.endswith("_depth_pcd.ply")
                                        and basename[:4].isdigit()
                                    ):
                                        source_seq = basename[:4]
                                    else:
                                        import re
                        ...
                        try:
                            ...
                            print(
                                f"配准错误: 批次 {i} 配准失败: {str(registration_error)}"
                            )
                            error_count += 1
                            dummy_vals = ["nan"] * 6
                            print(",".join(dummy_vals), file=fout)
                        ...
                        try:
                            ...
                            # 检查计算结果有效性
                            if not (
                                torch.isfinite(rot_norm).all()
                                and torch.isfinite(trans_norm).all()
                                and torch.isfinite(total_norm).all()
                            ):
                            ...
                            print(
                                f"误差计算错误: 批次 {i} 误差计算失败: {str(error_calc_error)}"
                            )
                            error_count += 1
                            dummy_vals = ["nan"] * 6
                            print(",".join(dummy_vals), file=fout)
                        ...
                        if do_visualize and current_vis_count < self.visualize_samples:
                            try:
                                ...
                                if hasattr(testloader.dataset, "get_cloud_info"):
                                    ...
                                    cloud_info = testloader.dataset.get_cloud_info(i)
                                    if cloud_info:
                                        identifier = cloud_info.get(
                                            "identifier", identifier
                                        )
                                        scene_name = cloud_info.get("scene", scene_name)
                                        source_seq = cloud_info.get(
                                            "sequence", source_seq
                                        )
                                        source_file_path = cloud_info.get("source_file")
                                        target_file_path = cloud_info.get("target_file")
                                    ...
                                    try:
                                        ...
                                        if hasattr(
                                            ...
                                            if idx < len(
                                                ...
                                                print(
                                                    f"获取到源文件: {source_file_path}"
                                                )
                                                print(
                                                    f"获取到目标文件: {target_file_path}"
                                                )
                                        # 然后检查子集数据集结构
                                        elif hasattr(
                                            testloader.dataset, "dataset"
                                            ...
                                            if hasattr(
                                                ...
                                                if idx < len(
                                                    ...
                                                    print(
                                                        f"从子集获取到源文件: {source_file_path}"
                                                    )
                                                    print(
                                                        f"从子集获取到目标文件: {target_file_path}"
                                                    )
                                    except Exception as e:
                                        print(f"获取文件路径时出错: {e}")
                                        traceback.print_exc()
                                ...
                                source_filename = os.path.join(
                                    self.vis_subdir,
                                    f"{scene_name}_{source_seq}_source.ply",
                                )
                                target_filename = os.path.join(
                                    self.vis_subdir,
                                    f"{scene_name}_{source_seq}_target.ply",
                                )

                                # 新增：原始源点云和目标点云的文件名
                                original_source_filename = os.path.join(
                                    self.vis_subdir,
                                    f"{scene_name}_{source_seq}_original_source.ply",
                                )
                                original_target_filename = os.path.join(
                                    self.vis_subdir,
                                    f"{scene_name}_{source_seq}_original_target.ply",
                                )
                                ...
                                try:
                                    with open(self.vis_log_file, "a") as f:
                                        # 记录基本信息
                                        predicted = ",".join(
                                            [f"{val:.6f}" for val in x_hat.tolist()]
                                        )
                                        gt_values = ",".join(
                                            [f"{val:.6f}" for val in (-mx_gt).tolist()]
                                        )

                                        # 记录文件信息和扰动详情
                                        f.write(
                                            f"{identifier},{self.current_pert_file},{predicted},{gt_values}\n"
                                        )
                                        ...
                                        f.write(
                                            f"源点云尺寸: {p1.shape}, 目标点云尺寸: {p0.shape}\n"
                                        )
                                        f.write(
                                            f"配准误差: 旋转={rot_norm.item():.6f}, 平移={trans_norm.item():.6f}, 总误差={total_norm.item():.6f}\n\n"
                                        )

                                    print(
                                        f"已将配准信息记录到日志文件: {self.vis_log_file}"
                                    )
                                except Exception as e:
                                    print(f"写入日志文件时出错: {e}")
                                    traceback.print_exc()
                                ...
                                # 复制原始PLY文件到可视化目录
                                success_copy = True
                                if source_file_path and os.path.exists(
                                    source_file_path
                                ):
                                    try:
                                        print(
                                            f"复制源点云: {source_file_path} -> {source_filename}"
                                        )
                                    ...
                                    print(
                                        f"警告: 无法找到源点云文件路径: {source_file_path}"
                                    )
                                    success_copy = False

                                if target_file_path and os.path.exists(
                                    target_file_path
                                ):
                                    try:
                                        print(
                                            f"复制目标点云: {target_file_path} -> {target_filename}"
                                        )
                                    ...
                                    print(
                                        f"警告: 无法找到目标点云文件路径: {target_file_path}"
                                    )
                                    success_copy = False
                                ...
                                try:
                                    ...
                                    if hasattr(
                                        testloader.dataset, "get_original_clouds"
                                    ):
                                        original_source, original_target = (
                                            testloader.dataset.get_original_clouds(i)
                                        )
                                        if (
                                    ...
                                    if original_source is not None:
                                        create_ply(
                                            original_source, original_source_filename
                                        )
                                    if original_target is not None:
                                        create_ply(
                                            original_target, original_target_filename
                                        )
                                    ...
                                    with open(self.vis_log_file, "a") as f:
                                        if (
                                            ...
                                            f.write(
                                                f"已保存原始点云数据到: {os.path.basename(original_source_filename)} 和 {os.path.basename(original_target_filename)}\n\n"
                                            )
                                        else:
                                ...
                                current_vis_count += 1
                                print(
                                    f"已完成样本 {identifier} 的可视化 ({current_vis_count}/{self.visualize_samples})"
                                )
                                ...
                                try:
                                    with open(self.vis_log_file, "a") as f:
                                        f.write(
                                            f"错误处理样本_{i},{self.current_pert_file},错误信息: {str(e)}\n"
                                        )
                                        f.write(
                        ...
                        # 打印进度
                        print(
                            f"测试: [{i + 1}/{total_samples}] {(i + 1) / total_samples * 100:.1f}% | "
                        ...
                        LOGGER.info(
                            "test, %d/%d, rot_error: %f, trans_error: %f, total_error: %f",
                            i,
                            total_samples,
                            rot_norm.item(),
                            trans_norm.item(),
        ...
        with open(self.filename, "a") as fout:
            ...
            print(
                f"# Successfully processed samples: {success_count}/{total_count} ({success_count / total_count * 100:.1f}%)",
                file=fout,
            )
            ...
            if success_count > 0:
                ...
                median_rot_error = (
                    all_rot_errors[len(all_rot_errors) // 2] if all_rot_errors else 0
                )
                median_trans_error = (
                    all_trans_errors[len(all_trans_errors) // 2]
                    if all_trans_errors
                    else 0
                )
                median_total_error = (
                    all_total_errors[len(all_total_errors) // 2]
                    if all_total_errors
                    else 0
                )
                ...
                if len(all_rot_errors) > 1:
                    std_rot_error = math.sqrt(
                        sum((x - avg_rot_error) ** 2 for x in all_rot_errors)
                        / (len(all_rot_errors) - 1)
                    )
                    std_trans_error = math.sqrt(
                        sum((x - avg_trans_error) ** 2 for x in all_trans_errors)
                        / (len(all_trans_errors) - 1)
                    )
                    std_total_error = math.sqrt(
                        sum((x - avg_total_error) ** 2 for x in all_total_errors)
                        / (len(all_total_errors) - 1)
                    )
                else:
                    std_rot_error = 0
                    std_trans_error = 0
                ...
                print(
                    f"# Median translation error: {median_trans_error:.6f}", file=fout
                )
                ...
                print(
                    f"# Min translation error: {min(all_trans_errors):.6f}", file=fout
                )
                print(
                    f"# Max translation error: {max(all_trans_errors):.6f}", file=fout
                )
                print(f"# Min total error: {min(all_total_errors):.6f}", file=fout)
                print(f"# Max total error: {max(all_total_errors):.6f}", file=fout)
            else:
                print(
                    f"# No successfully processed samples, cannot calculate error statistics",
                    file=fout,
                )

            if error_count > 0:
                print(
                    f"# Registration failed samples: {error_count}/{success_count} ({error_count / success_count * 100:.1f}%)",
                    file=fout,
                )
        ...
        print(f"\n====== Test Completed ======")
        print(
            f"Total time: {total_time:.2f} seconds (average {total_time / total_count:.2f} seconds per sample)"
        )
        print(
            f"Successfully processed samples: {success_count}/{total_count} ({success_count / total_count * 100:.1f}%)"
        )
        ...
        if need_visualization:
            ...
            print(
                f"Visualized {current_vis_count} samples for perturbation file {self.current_pert_file}"
            )
    ...
    def test_metrics(
        self, rotations_gt, translation_gt, rotations_ab, translation_ab, filename
    ):
        ...
        # root square error
        rot_err = np.sqrt(
            np.mean((np.degrees(rotations_ab) - np.degrees(rotations_gt)) ** 2, axis=1)
        )
        ...
        rot_mae_ab = np.mean(
            np.abs(np.degrees(rotations_ab) - np.degrees(rotations_gt))
        )
        ...
        # 2. use median error
        rot_mse_ab_02 = np.median(
            (np.degrees(rotations_ab) - np.degrees(rotations_gt)) ** 2
        )
        rot_rmse_ab_02 = np.sqrt(rot_mse_ab_02)
        rot_mae_ab_02 = np.median(
            np.abs(np.degrees(rotations_ab) - np.degrees(rotations_gt))
        )
...
def get_datasets(args):
    ...
    if args.perturbations:
        ...
        if (
            perturbation_filename == "gt_poses.csv"
            or "gt_poses" in perturbation_filename
        ):
    ...
    if args.dataset_type == "modelnet":
        ...
        if is_gt_poses_mode:
            ...
            testset = (
                ptlk.data.datasets.CADset4tracking_fixed_perturbation_random_sample(
        ...
        if use_voxelization:
            ...
            print(
                f"每个体素最大点数={voxel_config.max_voxel_points}, 最大体素数量={voxel_config.max_voxels}"
            )
        ...
        if args.pair_mode == "scene_reference":
            ...
            print(f"Target point cloud directory: {target_path}")
            print(
                f"Pairing method: Each source point cloud matches target point cloud with corresponding frame number"
            )
        ...
        if len(c3vd_dataset.pairs) == 0:
            print(
                f"Error: No paired point clouds found, please check pairing mode and data paths"
            )
            # 输出详细的调试信息
            ...
            if os.path.exists(source_root):
                ...
                if scenes:
                    for scene in scenes[
                        :2
                    ]:  # 只显示前两个场景的信息 # Only show info for first two scenes
                        scene_dir = os.path.join(source_root, scene)
                        files = os.listdir(scene_dir)
                        print(
                            f"Files in scene {scene}: {files[:5]}{'...' if len(files) > 5 else ''}"
                        )
            ...
            raise RuntimeError(
                "Cannot find paired point clouds, please check dataset structure and pairing mode settings"
            )
        ...
        if len(testdata) > 0:
            ...
            raise RuntimeError(
                "No test samples found after applying scene-based splitting"
            )
            ...
            def __call__(self, tensor):
                ...
                try:
                    ...
                    twist = (
                        torch.from_numpy(
                            numpy.array(
                                self.perturbations[self.current_perturbation_index]
                            )
                        )
                        .contiguous()
                        .view(1, 6)
    ...
    if (
        ...
        if len(testset) > args.max_samples:
            print(
                f"限制测试集样本数到 {args.max_samples} 个（原样本数：{len(testset)}）"
            )
            testset = torch.utils.data.Subset(testset, range(args.max_samples))
            print(f"限制后样本数: {len(testset)}")
        else:
            print(
                f"测试集样本数 {len(testset)} 小于或等于最大样本数限制 {args.max_samples}，使用全部样本"
            )
    else:
...
def process_single_pair(args):
    ...
    try:
        ...
        # 解析扰动值
        perturbation_values = [
            float(x.strip()) for x in args.single_perturbation.split(",")
        ]
        if len(perturbation_values) != 6:
            raise ValueError(
                f"扰动值必须是6个数字 (rx,ry,rz,tx,ty,tz)，当前提供了{len(perturbation_values)}个"
            )
        ...
        with torch.no_grad():
            for i, (template, source, igt) in enumerate(testloader):
                ...
                print(
                    f"  向量形式: [{', '.join([f'{x:.6f}' for x in perturbation_values])}]"
                )
                print(
                    f"  旋转部分 (rx,ry,rz): [{', '.join([f'{x:.6f}' for x in perturbation_values[:3]])}]"
                )
                print(
                    f"  平移部分 (tx,ty,tz): [{', '.join([f'{x:.6f}' for x in perturbation_values[3:]])}]"
                )
                ...
                for row in range(4):
                    print(
                        f"    [{', '.join([f'{g_hat_np[row, col]:8.6f}' for col in range(4)])}]"
                    )

                # 提取旋转和平移
                predicted_twist = se3.log(g_hat).cpu().numpy().squeeze()
                print(
                    f"  扭转向量形式: [{', '.join([f'{x:.6f}' for x in predicted_twist])}]"
                )
                print(
                    f"  旋转部分: [{', '.join([f'{x:.6f}' for x in predicted_twist[:3]])}]"
                )
                print(
                    f"  平移部分: [{', '.join([f'{x:.6f}' for x in predicted_twist[3:]])}]"
                )
                ...
                print(
                    f"  旋转误差: {rotation_error_rad:.6f} 弧度 = {rotation_error_deg:.6f} 度"
                )
                ...
                if args.enhanced_output:
                    ...
                    with open(args.outfile, "w") as f:
                        ...
                        f.write(
                            f"input_perturbation_vector,{','.join([f'{x:.6f}' for x in perturbation_values])}\n"
                        )
                        f.write(
                            f"input_rotation_part,{','.join([f'{x:.6f}' for x in perturbation_values[:3]])}\n"
                        )
                        f.write(
                            f"input_translation_part,{','.join([f'{x:.6f}' for x in perturbation_values[3:]])}\n"
                        )
                        f.write("\n")

                        f.write("# 预测的变换 Predicted Transformation\n")
                        f.write(
                            f"predicted_twist_vector,{','.join([f'{x:.6f}' for x in predicted_twist])}\n"
                        )
                        f.write(
                            f"predicted_rotation_part,{','.join([f'{x:.6f}' for x in predicted_twist[:3]])}\n"
                        )
                        f.write(
                            f"predicted_translation_part,{','.join([f'{x:.6f}' for x in predicted_twist[3:]])}\n"
                        )
                        f.write("\n")
                        ...
                        for row in range(4):
                            f.write(
                                f"transformation_matrix_row_{row},{','.join([f'{g_hat_np[row, col]:.6f}' for col in range(4)])}\n"
                            )
                        f.write("\n")
