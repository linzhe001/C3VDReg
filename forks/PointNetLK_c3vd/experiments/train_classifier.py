...
def options(argv=None):
    ...
    )  # result: ${BASENAME}_feat_best.pth
    parser.add_argument(
        "-i",
        "--dataset-path",
        required=True,
...
def run(args, trainset, testset, action):
    ...
    print(
        f"Batch size: {args.batch_size}, Points per cloud: {args.num_points}, Drop last: {args.drop_last}"
    )
    ...
    if args.pretrained:
        ...
        # 如果是PointMamba模型，使用特殊的加载逻辑
        if args.model_type == "pointmamba":
            print(f"🔄 加载PointMamba预训练权重: {args.pretrained}")
            try:
                # 只加载特征提取器的权重
                _, utilization_rate = model.features.load_pretrained_weights(
                    args.pretrained, strict=False, verbose=True
                )
                print(
                    f"✅ PointMamba预训练权重加载成功，利用率: {utilization_rate:.1f}%"
                )
            except Exception as e:
                print(f"⚠️ PointMamba预训练权重加载失败: {e}")
                print("继续使用随机初始化权重")
        else:
            # 其他模型使用标准加载方式
    ...
    )  # Use pin_memory for acceleration
    ...
    if args.optimizer == "Adam":
        optimizer = torch.optim.Adam(
            learnable_params, lr=0.001, weight_decay=1e-4
        )  # 优化：增加权重衰减
    else:
        optimizer = torch.optim.SGD(
            learnable_params, lr=0.01, momentum=0.9, weight_decay=1e-4
        )  # 优化：增加权重衰减
    ...
    base_lr = (
        ...
        else (0.001 if args.optimizer == "Adam" else 0.01)
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            print(
                f"[Save] Found better model with validation loss: {val_loss:.4f} (accuracy: {val_info:.4f})"
            )
        ...
        if writer is not None:
            ...
            writer.add_scalar("Accuracy/train", running_info, epoch)
            writer.add_scalar("Accuracy/val", val_info, epoch)
            writer.add_scalar("Learning_Rate", current_lr, epoch)
            writer.add_scalar("Time/epoch", epoch_time, epoch)

        print(
            f"[Time] Epoch {epoch + 1}: {epoch_time:.2f} sec | Loss: {running_loss:.4f} | Validation: {val_loss:.4f} | Accuracy: {running_info:.2f} | LR: {current_lr:.8f}"
        )
        print(f"[Info] Best validation loss: {best_val_loss:.4f} at epoch {best_epoch}")

        LOGGER.info(
            "epoch, %04d, %f, %f, %f, %f, %04d, %f",
            epoch + 1,
            running_loss,
            val_loss,
            running_info,
...
class Action:
    ...
    def create_model(self):
        ...
        if self.model_type == "attention":
            ...
            return attention_v1.AttentionNet_classifier(
                self.num_classes, feat, self.dim_k
            )
        elif self.model_type == "mamba3d":
            # 创建Mamba3D模型
            ...
            return fast_point_attention.FastPointAttention_classifier(
                self.num_classes, feat, self.dim_k
            )
        elif self.model_type == "cformer":
            # 创建Cformer模型
            ...
            return pointmamba_adapter.PointMamba_classifier(
                self.num_classes, feat, self.dim_k
            )
        else:
            # 创建原始pointnet模型
    ...
    def train_1(self, model, trainloader, optimizer, device):
        ...
        for i, data in enumerate(trainloader):
            ...
            if not torch.isfinite(loss):
                print(
                    f"Warning: Batch {i} has non-finite loss value {loss.item()}, skipping"
                )
                nan_count += 1
                batch_start = time.time()
                continue
            ...
            for param in model.parameters():
                if param.grad is not None:
                    if (1 - torch.isfinite(param.grad).long()).sum() > 0:
                        do_step = False
                        print(
                            f"Warning: Found non-finite gradient values, skipping parameter update"
                        )
                        break
            if do_step:
                optimizer.step()
            ...
            if i % 5 == 0:
                if str(device) != "cpu":
                    ...
                    print(
                        f"Batch {i}/{len(trainloader)} | Loss: {loss.item():.4f} | GPU Memory: {mem_used:.1f}/{mem_total:.1f}MB | Time: {batch_time:.4f} sec"
                    )
                else:
                    print(
                        f"Batch {i}/{len(trainloader)} | Loss: {loss.item():.4f} | Time: {batch_time:.4f} sec"
                    )
        ...
        if nan_count > 0:
            print(
                f"Total of {nan_count} batches were skipped due to NaN loss ({nan_count / len(trainloader) * 100:.2f}%)"
            )
        ...
        print(
            f"Valid batches: {batch_count}/{len(trainloader)} ({batch_count / len(trainloader) * 100:.2f}%)"
        )
        print(
            f"Average batch time: {avg_batch:.4f} sec = Data loading: {avg_data:.4f} sec + Forward pass: {avg_forward:.4f} sec + Backward pass: {avg_backward:.4f} sec"
        )
    ...
    def eval_1(self, model, testloader, device):
            ...
            for i, data in enumerate(testloader):
                ...
                try:
                    ...
                    if not torch.isfinite(loss) or loss.item() > 1e6:
                        print(
                            f"Warning: Batch {i} has non-finite validation loss value {loss.item()}, skipping"
                        )
                        nan_count += 1
                        batch_start = time.time()
                        continue
                    ...
                    if i % 5 == 0 or i == len(testloader) - 1:
                        if str(device) != "cpu":
                            ...
                            print(
                                f"Validation batch {i}/{len(testloader)} | Loss: {loss_val:.4f} | Accuracy: {batch_acc:.4f} | GPU Memory: {mem_used:.1f}/{mem_total:.1f}MB | Time: {batch_time:.4f} sec"
                            )
                        else:
                            print(
                                f"Validation batch {i}/{len(testloader)} | Loss: {loss_val:.4f} | Accuracy: {batch_acc:.4f} | Time: {batch_time:.4f} sec"
                            )
        ...
        if nan_count > 0:
            print(
                f"Total of {nan_count} validation batches were skipped due to errors or non-finite loss ({nan_count / len(testloader) * 100:.2f}%)"
            )
        ...
        print(
            f"Valid batches: {batch_count}/{len(testloader)} ({batch_count / len(testloader) * 100:.2f}%)"
        )
        print(
            f"Average batch time: {avg_batch:.4f} sec = Data loading: {avg_data:.4f} sec + Forward pass: {avg_forward:.4f} sec"
        )
...
class C3VDDatasetForClassification(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path, transform=None, classinfo=None):
        ...
        print(
            f"目标点云目录 {self.target_root} 存在: {os.path.exists(self.target_root)}"
        )

        # 如果目录不存在，打印当前目录内容
        if not os.path.exists(self.source_root) or not os.path.exists(self.target_root):
            print(
                f"数据集根目录内容: {os.listdir(dataset_path) if os.path.exists(dataset_path) else '无法访问'}"
            )
        ...
        for scene in self.scene_to_prefix:
            ...
            if os.path.isdir(source_path):
                ...
                source_files = glob.glob(
                    os.path.join(source_path, "????_depth_pcd.ply")
                )
                ...
                if os.path.exists(self.source_root):
                    ...
                    if source_dirs:
                        ...
                        if os.path.isdir(first_scene):
                            print(
                                f"第一个场景目录 {first_scene} 内容: {os.listdir(first_scene)}"
                            )
...
def get_datasets(args):
    ...
    if args.dataset_type == "modelnet":
        ...
        c3vd_dataset = C3VDDatasetForClassification(
            args.dataset_path, transform=None, classinfo=cinfo
        )
        ...
        try:
            ...
            trainset = C3VDClassifierDataset(
                dataset=train_subset, num_points=args.num_points
            )

            testset = C3VDClassifierDataset(
                dataset=test_subset, num_points=args.num_points
            )
        except Exception as e:
