...
def options(argv=None):
    ...
    parser.add_argument(
        "--drop-last", action="store_true", help="丢弃不完整的最后一个批次"
    )
...
def run(args, trainset, testset, action):
    ...
    print(
        f"批次大小: {args.batch_size}, 点云数: {args.num_points}, drop_last: {args.drop_last}"
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print(
            f"[时间] 第{epoch + 1}轮训练: {epoch_time:.2f}秒 | 损失: {running_loss:.4f} | 验证: {val_loss:.4f} | 准确率: {running_info:.2f}"
        )
        ...
        print(
            f"[进度] {epoch + 1}/{args.epochs} 轮 | 已用时间: {elapsed / 60:.1f}分钟 | 预计剩余: {remaining / 60:.1f}分钟"
        )
        ...
        for i, data in enumerate(trainloader):
            ...
            if i % 5 == 0:
                if str(device) != "cpu":
                    ...
                    print(
                        f"批次 {i}/{len(trainloader)} | 损失: {loss.item():.4f} | GPU内存: {mem_used:.1f}/{mem_total:.1f}MB | 时间: {batch_time:.4f}秒"
                    )
                else:
                    print(
                        f"批次 {i}/{len(trainloader)} | 损失: {loss.item():.4f} | 时间: {batch_time:.4f}秒"
                    )
...
def get_datasets(args):
    ...
    if args.dataset_type == "modelnet":
        ...
        # 创建C3VD分类数据集
        dataset = C3VDDatasetForClassification(
            args.dataset_path, transform=transform, classinfo=cinfo
        )
