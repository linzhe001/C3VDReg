...
def parse_args():
    ...
    parser.add_argument("--dataset-path", type=str, required=True, help="数据集根路径")
    parser.add_argument(
        "--categoryfile",
        type=str,
        default="",
    ...
    parser.add_argument(
        "--reference-name", type=str, default=None, help="C3VD 场景参考点云名称"
    )
    parser.add_argument("--num-points", type=int, default=1024, help="每个点云点数")
    parser.add_argument(
        "--use-voxelization",
        action="store_true",
        default=False,
    ...
    parser.add_argument(
        "--mag", type=float, default=0.8, help="C3VD 随机变换幅度 (twist-scale)"
    )
    # DCP 模型参数
    parser.add_argument(
        "--emb-dims", type=int, default=512, help="特征维度 (默认: 512)"
    )
    parser.add_argument(
        "--emb-nn",
        type=str,
    ...
    parser.add_argument(
        "--head", type=str, default="mlp", choices=["mlp", "svd"], help="头部类型"
    )
    parser.add_argument("--cycle", action="store_true", help="使用 cycle 约束")
    # Transformer 网络参数
    parser.add_argument(
        "--n-blocks", type=int, default=1, help="Transformer编码解码层数 (默认: 1)"
    )
    parser.add_argument(
        "--n-heads", type=int, default=4, help="多头注意力头数 (默认: 4)"
    )
    parser.add_argument(
        "--ff-dims",
        type=int,
    ...
    parser.add_argument(
        "--dropout", type=float, default=0.0, help="Transformer中丢弃率 (默认: 0.0)"
    )
    ...
    parser.add_argument(
        "--save-dir",
        type=str,
        default="./experiments/checkpoints/dcp",
        help="模型保存目录",
    )
    parser.add_argument("--resume", type=str, default="", help="断点恢复模型路径")
...
def eval_one_epoch(model, loader, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            rot_ab_gt = igt[:, :3, :3]
            trans_ab_gt = igt[:, :3, 3]
            loss = F.mse_loss(rot_ab_pred, rot_ab_gt) + F.mse_loss(
                trans_ab_pred, trans_ab_gt
            )
            # 计算对齐误差
            aligned = transform_point_cloud(src, rot_ab_pred, trans_ab_pred).transpose(
                1, 2
            )
...
def main():
    ...
    if args.dataset_type == "modelnet":
        ...
        train_raw, val_raw = torch.utils.data.random_split(
            base_dataset, [train_count, val_count]
        )
        # 体素化配置
        voxel_config = None
