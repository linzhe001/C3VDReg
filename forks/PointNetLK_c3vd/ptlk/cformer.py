...
class MLPNet(torch.nn.Module):
    ...
    def __init__(
        self, nch_input, nch_layers, b_shared=True, bn_momentum=0.1, dropout=0.0
    ):
        super().__init__()
...
class GlobalCrossAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        distribute_attn = (
            torch.matmul(distribute_q, distribute_k.transpose(-2, -1)) / self.scale
        )
...
class CFormer_features(torch.nn.Module):
    ...
    def __init__(
        self, dim_k=1024, sym_fn=symfn_max, scale=1, num_proxy_points=8, num_blocks=2
    ):
        super().__init__()
        ...
        self.local_blocks = nn.ModuleList(
            [LocalSelfAttention(self.d_model) for _ in range(num_blocks)]
        )
...
class CFormer_classifier(torch.nn.Module):
    ...
    def __init__(self, num_c, cfeat, dim_k):
        ...
        # 分类头：特征向量 -> 分类结果
        list_layers = mlp_layers(
            dim_k, [512, 256], b_shared=False, bn_momentum=0.1, dropout=0.0
        )
