...
from . import pointmamba_adapter
...
class PointLK(torch.nn.Module):
    ...
    @staticmethod
    def do_forward(
        net, p0, p1, maxiter=10, xtol=1.0e-7, p0_zero_mean=True, p1_zero_mean=True
    ):
        # 构造变换矩阵并 clone，避免 expand 导致的内存共享
        a0 = (
            torch.eye(4).view(1, 4, 4).expand(p0.size(0), 4, 4).to(p0).clone()
        )  # [B, 4, 4]
        a1 = (
            torch.eye(4).view(1, 4, 4).expand(p1.size(0), 4, 4).to(p1).clone()
        )  # [B, 4, 4]
        if p0_zero_mean:
    ...
    def approx_Jic(self, p0, f0, dt):
        ...
        f = (
            (f_out[0] if isinstance(f_out, tuple) else f_out)
            .view(batch_size, 6, -1)
            .transpose(1, 2)
        )  # [B, K, 6]
